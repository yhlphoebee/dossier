...
def _select_system_prompt(agent: str | None) -> str:
    agent_lower = (agent or "").lower()
    if agent_lower == "strategy":
        return STRATEGIST_SYSTEM_PROMPT
    if agent_lower == "research":
        return INVESTIGATOR_SYSTEM_PROMPT
    if agent_lower == "concept":
        return VISUAL_DIRECTOR_SYSTEM_PROMPT
    if agent_lower == "present":
        return NARRATOR_SYSTEM_PROMPT
    # Fallback: strategist as a reasonable default
    return STRATEGIST_SYSTEM_PROMPT


def build_system_prompt(project: Project, agent: str | None = None) -> str:
    """
    System prompt = agent-specific role description + project context (title & description).
    Injected once as the system message so the model treats it as persistent memory.
    """
    base = _select_system_prompt(agent).strip()
...
def build_messages(
    ...
    """
    system_prompt = build_system_prompt(project, agent=agent)
